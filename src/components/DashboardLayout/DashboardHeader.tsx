...
import {
  Menu, Bell, User, ChevronDown, LogOut, Settings, HelpCircle, Sparkles, X, Edit,
} from "lucide-react";
...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  return (
    <>
      <header className={`bg-white dark:bg-dark-800 border-b border-gray-200 dark:border-dark-700 shadow-lg dark:shadow-dark-900 ${className}`}>
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-dark-800 rounded-xl shadow-2xl border border-gray-200 dark:border-dark-700 z-50 overflow-hidden animate-in slide-in-from-top-5 duration-300">
                    <div className="p-4 border-b border-gray-200 dark:border-dark-700 bg-gradient-to-r from-primary/5 to-purple-500/5 dark:from-primary/10 dark:to-purple-500/10">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setShowNotifications(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-dark-700 rounded-lg transition-colors">
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {notifications.map((n) => (
                        <div key={n.id} className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-dark-700 border-b border-gray-100 dark:border-dark-700 last:border-b-0 transition-all duration-300 group relative ${!n.read ? "bg-blue-50 dark:bg-blue-900/10" : ""}`}>
                          {!n.read && <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-blue-500 to-cyan-500" />}
                          <div className="flex items-start space-x-3">
                            <div className={`p-2 rounded-lg ${!n.read ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gray-100 dark:bg-dark-700'}`}>
                              <Bell className={`h-4 w-4 ${!n.read ? 'text-blue-500' : 'text-gray-500 dark:text-gray-400'}`} />
                            </div>
                    ...
                    <div className="p-3 border-t border-gray-200 dark:border-dark-700 text-center">
                      <Link to="/dashboard/notifications" className="text-sm font-medium relative inline-flex items-center group" onClick={() => setShowNotifications(false)}>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden lg:block text-left relative z-10">
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{displayUser?.name || "User"}</p>
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-dark-800 rounded-xl shadow-2xl border border-gray-200 dark:border-dark-700 z-50 overflow-hidden animate-in slide-in-from-top-5 duration-300">
                    <div className="p-4 border-b border-gray-200 dark:border-dark-700 bg-gradient-to-r from-primary/5 to-purple-500/5 dark:from-primary/10 dark:to-purple-500/10">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-gray-900 dark:text-white">{displayUser?.name || "User"}</p>

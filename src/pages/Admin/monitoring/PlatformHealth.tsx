...
import { 
  Activity, Heart, Server, Database, Cloud, Wifi, CheckCircle, 
  XCircle, AlertCircle, Clock, RefreshCw, TrendingUp, TrendingDown,
  Cpu, HardDrive, Zap, Shield, Eye, Bell, Calendar, Download
} from 'lucide-react';
...
const PlatformHealth: React.FC = () => {
  ...
  const fetchHealthData = async () => {
    ...
    try {
      ...
      // Update metrics with random status for demo
  ...
  const getStatusBadge = (status: string) => {
    const configs: Record<string, { icon: any; color: string; bg: string }> = {
      healthy: { icon: CheckCircle, color: 'text-green-700', bg: 'bg-green-100' },
      degraded: { icon: AlertCircle, color: 'text-yellow-700', bg: 'bg-yellow-100' },
      down: { icon: XCircle, color: 'text-red-700', bg: 'bg-red-100' },
      warning: { icon: AlertCircle, color: 'text-orange-700', bg: 'bg-orange-100' }
    };
  ...
  const getOverallStatus = () => {
    const configs: Record<string, { icon: any; color: string; bg: string; text: string }> = {
      healthy: { icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-50', text: 'All systems operational' },
      degraded: { icon: AlertCircle, color: 'text-yellow-600', bg: 'bg-yellow-50', text: 'Partial system degradation' },
      down: { icon: XCircle, color: 'text-red-600', bg: 'bg-red-50', text: 'Major system outage' }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <Activity className="h-8 w-8" style={{ color: deepTeal }} />
            Platform Health
          </h1>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Server className="h-5 w-5" style={{ color: deepTeal }} />
              <h2 className="text-lg font-semibold text-gray-900">Service Health</h2>
            </div>
            <ReusableButton size="sm" variant="secondary" icon={Download}>
              Export Report
            </ReusableButton>
          </div>
          <ReusableTable
